...
if(isset($_GET['pid'])){
    ...
    $query1 = "SELECT firstname,lastname FROM users WHERE email = '$client_email'";
    $result0=mysqli_query($conn,$query1);
...
if(isset($_POST['accept'])){
    $q="UPDATE project SET is_approved='yes',dev='$dev_email' WHERE pid='$id1'";
    $res=mysqli_query($conn,$q);
    header('Location:devviewproject.php?pid='.$id1.'&accept=success');
}
if(isset($_POST['cancel'])){
    $q="UPDATE project SET is_approved='no',dev='no_dev' WHERE pid='$id1'";
    $res=mysqli_query($conn,$q);
    header('Location:devhome.php?project canceled');
}
if(isset($_POST['submit_project'])){
    $q="UPDATE project SET is_submitted='yes' WHERE pid='$id1'";
    $res=mysqli_query($conn,$q);
    header('Location:devviewproject.php?pid='.$id1.'&submit=success');
}
?>
<!DOCTYPE html>
<html lang="en">
        ...
        <?php
            echo'
                <div>
                    ...
                    <span class="lead">Posted By: </span><span class=detail>'.$row0['firstname'].' '.$row0['lastname'].'</span><br>
                    <span class="lead">Email: </span><span class="detail">'.$row1['email'].'</span><br> 
                    ...
                    <hr>
                    <span class="lead">References</span>
                    ...
                    <hr>';
                    if($row1['is_approved']=='no'){
                        echo'<div class="d-flex mt-2">
                                <div class="ml-auto">
                                    <form action="" method="POST"><button type="submit"name ="accept" class="btn btn-success ml-auto">Accept</button></form>
                                </div>
                            </div>';
                    }
                    if($row1['is_approved']=='yes'&&$row1['is_completed']=='no'){
                        echo'
                        <span class="lead">Project Files</span>
                        <div class="rounded border p-2">';
                        $sql3="SELECT * FROM devfile where id='$id1'";
                        $result3=mysqli_query($conn,$sql3);
                        if($result3){
                            echo'<div class="justify-content-start d-flex flex-wrap" style="max-width:1040px;">';
                            while($row3=mysqli_fetch_array($result3)){
                                $file_name_enc=$row3['fname'];
                                $file_name=explode(".",$file_name_enc);
                                $file_name_final=$file_name[2].'.'.$file_name[3];
                                echo'
                                    <div style="width:70px;white-space:nowrap;text-overflow:ellipsis;overflow:hidden;background-color:#efefef;" class="border rounded m-3 pt-3 pl-1 pr-1 pb-1" title="'.$file_name_final.'">
                        ...
                        echo'
                             <form action="../php/newdevupload.php?pid='.$id1.'" method="Post" enctype="multipart/form-data">
                                <div style="width:70px;white-space:nowrap;text-overflow:ellipsis;overflow:hidden;background-color:#efefef;" class="border rounded m-3 pt-3 pl-1 pr-1 pb-1" title="'.$file_name_final.'">
                        ...
                        <div class="d-flex mt-2">
                            <div>
                                <form action="" method="POST"><button type="submit"name ="cancel" class="btn btn-danger ">Decline Project</button></form>
                            </div>
                            <div class="ml-auto">
                                <form action="" method="POST"><button type="submit"name ="submit_project" class="btn btn-primary">Submit Project</button></form>
                            </div>
                        </div>';
                    }
                    else if($row1['is_completed']=='yes'){
                        echo'
                        <span class="lead">Project Files</span>
                        <div class="rounded border p-2">';
                        $sql3="SELECT * FROM devfile where id='$id1'";
                        ...
                        <div class="d-flex mt-2">
                            <div class="ml-auto">
                                <button type="button" onclick="window.location.href=\'devhome.php\'" class="btn btn-success"><i class="far fa-hand-point-left"></i><i class="far fa-smile-wink"></i></button>
                            </div>
                        </div>';
                    }
                    echo'
